...
async function runEval(): Promise<void> {
  ...
  for (let i = 0; i < dataset.length; i++) {
    ...
    try {
      ...
      console.log(
        `${statusIcon(result.passed)}  ${recallStr}  ${mrrStr}  ${latencyStr}  ${colorize(`(${result.returnedKeys.length} hits)`, 'dim')}`,
      );
    } catch (err) {
      const latencyMs = Date.now() - start;
...
function printSummary(summary: EvalSummary): void {
  ...
  console.log(
    `  Failed:          ${colorize(String(summary.failed), summary.failed > 0 ? 'red' : 'green')}`,
  );

  console.log(colorize('\n  ── Key Metrics ──', 'cyan'));
  console.log(
    `  Recall@1:        ${colorize(formatPercent(summary.recallAt1), metricColor(summary.recallAt1, 0.8))}  (target ≥ 80%)`,
  );
  console.log(
    `  Recall@5:        ${colorize(formatPercent(summary.recallAt5), metricColor(summary.recallAt5, 0.9))}  (target ≥ 90%)`,
  );
  console.log(
    `  Recall@10:       ${colorize(formatPercent(summary.recallAt10), metricColor(summary.recallAt10, 0.95))}  (target ≥ 95%)`,
  );
  console.log(
    `  Recall@20:       ${colorize(formatPercent(summary.recallAt20), metricColor(summary.recallAt20, 0.95))}  (target ≥ 95%)`,
  );
  console.log(
    `  Precision@5:     ${colorize(formatPercent(summary.precisionAt5), metricColor(summary.precisionAt5, 0.7))}  (target ≥ 70%)`,
  );
  console.log(
    `  MRR@10:          ${colorize(formatPercent(summary.mrrAt10), metricColor(summary.mrrAt10, 0.8))}  (target ≥ 80%)`,
  );
  console.log(
    `  NDCG@10:         ${colorize(formatPercent(summary.ndcgAt10), metricColor(summary.ndcgAt10, 0.8))}  (target ≥ 80%)`,
  );
  console.log(
    `  Exact Key Hit:   ${colorize(formatPercent(summary.exactKeyHitRate), metricColor(summary.exactKeyHitRate, 1.0))}  (target = 100%)`,
  );
  console.log(
    `  Type Accuracy:   ${colorize(formatPercent(summary.typeAccuracy), metricColor(summary.typeAccuracy, 0.95))}  (target ≥ 95%)`,
  );
  console.log(
    `  Empty Result %:  ${colorize(formatPercent(summary.emptyResultRate), summary.emptyResultRate <= 0.05 ? 'green' : 'yellow')}  (target ≤ 5%)`,
  );
  console.log(`  P50 Latency:     ${colorize(`${summary.p50Latency}ms`, 'dim')}`);
  console.log(
    `  P95 Latency:     ${colorize(`${summary.p95Latency}ms`, metricColor(800 - summary.p95Latency, 0))}  (target ≤ 800ms)`,
  );
  ...
  for (const [cat, stats] of Object.entries(summary.byCategory)) {
    ...
    const bar =
      rate >= 0.8
        ? colorize(`${stats.passed}/${stats.total}`, 'green')
        : colorize(`${stats.passed}/${stats.total}`, 'yellow');
  ...
  const bucketCategories = Object.keys(summary.buckets).filter(
    (k) => k.startsWith('lang:') || k.startsWith('len:'),
  );
  if (bucketCategories.length > 0) {
    ...
    for (const bucketKey of bucketCategories) {
      ...
      console.log(
        `    ${bucketKey.padEnd(20)} n=${String(b.count).padEnd(4)} R@5=${formatPercent(b.recallAt5).padEnd(7)} MRR=${formatPercent(b.mrrAt10).padEnd(7)} NDCG=${formatPercent(b.ndcgAt10)}`,
      );
    }
  }
  ...
  if (failedCases.length > 0) {
    ...
    for (const r of failedCases) {
      const reasonTag =
        r.failureReason !== 'none' ? colorize(` [${r.failureReason}]`, 'yellow') : '';
      console.log(
        `    ${colorize(r.id, 'red')}: R@5=${formatPercent(r.recallAtK)} MRR=${formatPercent(r.mrrAt10)} returned=[${r.returnedKeys.slice(0, 3).join(', ')}] expected=[${r.expectedKeys.join(', ')}]${reasonTag}`,
      );
    }
  }
