<script lang="ts">
  ...
  let { lang, tr, selectedTable, ontablechange, ontogglelang }: Props = $props();
...
<header>
  ...
  <span>v1</span>
  <select class="table-select" value={selectedTable} onchange={(e) => ontablechange((e.target as HTMLSelectElement).value)}>
    {#each tables as opt}
      <option value={opt.value}>{opt.label}</option>
    {/each}
  </select>
  <a class="github-link" href="https://github.com/rwr-infra/rwr-data-agent" target="_blank" rel="noopener noreferrer" aria-label="GitHub">
  ...
  <button class="lang-btn" onclick={ontogglelang}>{tr.langLabel}</button>
</header>
