...
- **A flip is written to the session immediately** (`persistTogglesToSession`), not at the next `saveCurrentSession`: that one early-returns on a thread with no messages, so a toggle set before the first question would be lost on navigating away. It deliberately does not touch `updatedAt` — flipping a switch is not activity, and bumping it would reorder the session list under the user.
