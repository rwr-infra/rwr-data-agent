...
| Route | Purpose |
|---|---|
| `POST /v1/chat/completions` | Main endpoint. Extra fields: `mod`, `response_format`, `mode` ('max' = best-of-N), `candidates`, `self_check` (opt into reflection) |
...
### Streaming format
Custom **NDJSON**, not SSE — the OpenAI-shaped path is a naming coincidence, not a wire promise, and a real OpenAI client cannot read this stream. One JSON object per line, keyed by `type`: `turn-start`, `text-delta`, `reasoning-delta`, `json-delta`, `tool-step`, `steer-applied`, `reflection-start`, `reflection`, `revision`, `finish`, `error`, `ping`. Treat these shapes as a contract with the Web UI — **extend with new optional fields, never repurpose an existing one.**
...
- `reflection-start` — the self-check has begun, `trigger[]` only. It exists because that phase emits nothing else for up to a minute: the answer is complete, the stop button is lit, and a client with no label for it shows what looks like a hang. **Not a promise of a verdict** — a check that fails emits no `reflection` at all, so `finish` is the only terminator a client can rely on.
...
- **Requested per message, then still conditional.** `body.self_check` is the client's opt-in — the UI's 自检 / Self-check toggle, next to Max in the composer — and there is deliberately **no operator master switch** behind it: the check costs one LLM round trip on the turn the user is already waiting on, so the decision belongs to whoever pays for it. An omitted field therefore means off, so a bare API client never reflects — `REFLECTION_ENABLED` is read nowhere now and `validateConfig()` warns when a stale `.env` still carries it. The eval harness opts in explicitly on `agent-inheritance-g36`, which is the only end-to-end cover the feature has.
  - After the opt-in, `shouldReflect()` still decides: it fires on a failed tool call, on `step-limit`, or on an inheritance/enumeration intent — and refuses outright on a user stop, a vanished client, or an empty answer that `step-limit` does not explain. `output-limit` is not a signal: a rewrite would hit the same output cap. `DEBUG_REFLECTION=force` fires it on any *otherwise eligible* turn — it relaxes the risk signals, not the opt-in, so a turn that did not ask for the check still gets none.
...
- **Covers the single path and max mode; structured output is excluded** — zod already constrains that shape, and its content is assembled from the retrieved context rather than narrated. In max mode the transcript is the candidates' own: each `BestOfNCandidateResult` carries a `toolTranscript` (`src/agent/synthesize.ts`) and the route flat-maps them into the check, every line tagged with its candidate index — so the reflection sees what the drafts actually did, not just the shared retrieval context. `BestOfNCandidateResult.toolFailures` is what carries the risk signal out.
...
- **UI**: while the check runs, the chat's tail status line reads "正在自检答案…" (`reflection-start` sets it, `reflection` or `finish` clears it). That line is present for **every** unfinished phase, not just this one — the skeleton indicator only ever covered the gap before the first block, so everything after it used to be silent. A clean check is a one-line badge. A revision renders as an **ordinary AI bubble** — not a collapsible panel or a quote — because it *is* the answer the conversation continues from; framing it as an aside made the superseded text look like the real answer. It therefore takes over the turn's action bar and meta line (`turnEndIds` counts a revision and, being later, it wins), and the turn's copy yields it. Findings collapse into one note above it, **aggregated by code** (`缺少证据支撑的结论 ×10`): a reflection routinely reports the same finding about several claims, and one badge each filled the header with a dozen identical chips. The per-claim `detail` stays on the wire.
...
**Two per-message toggles live in the composer's stats row** — Self-check (`body.self_check`) and Max (`body.mode: 'max'`), in that order because that is their cost order. Both persist the same way, and it is a double write: `localStorage` (the `rwr-data-agent-cache` blob) holds the global default a fresh session starts from, while `Session.selfCheck` / `Session.maxMode` hold what each conversation was using — restoring a session adopts its values *and* refreshes the cache, so switching back to an old thread brings its own settings with it and the cache never goes stale against what is on screen. Both default off; a new toggle should follow the same touch points in `App.svelte` rather than inventing a third persistence style.

Two details there are load-bearing, and both were bugs first:
- **The boolean is stored as itself, never `\|\| undefined`.** The restore guards read `!== undefined`, so collapsing `false` to absent makes a session the user explicitly switched *off* inherit `true` from whichever thread was open before. `selectedMod` still uses `\|\| undefined` — there the empty string genuinely means "no package chosen".
- **A flip is written to the session immediately** (`persistTogglesToSession`), not at the next `saveCurrentSession`: that one early-returns on a thread with no messages, so a toggle set before the first question would be lost on navigating away. It writes with `saveSession(s, { touch: false })`: that function restamps `updatedAt` by default, and the drawer is ordered by it, so a plain save would jump the thread to the top of the list for a switch the user flipped.
...
| `REFLECTION_MODEL` | `LLM_MODEL` | Model for the reflection call; pinned only when set explicitly, otherwise follows `body.model`. Whether reflection runs at all is the client's per-message call (`body.self_check`) — there is no env switch for it. See [Reflection](#reflection-post-answer-self-critique) |
| `LANGFUSE_*` | disabled | |
